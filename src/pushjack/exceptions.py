...
class ServerError(NotificationError):
    ...
    def __init__(self, identifier):
        super(ServerError, self).__init__(self.code, self.description, identifier)
        self.identifier = identifier

    def __str__(self):  # pragma: no cover
        return "{0} (code={1}): {2} for identifier {3}".format(
            self.__class__.__name__, self.code, self.description, self.identifier
        )
...
class APNSUnsendableError(APNSServerError):
    """Exception for when notification can't be send due to previous error for another
    notification."""
...
class Raiser(object):
    """Helper class for raising an exception based on error class name prefix and
    exception code."""
...
def map_errors(prefix):
    ...
    for name, obj in iteritems(globals()):
        if name.startswith(prefix) and getattr(obj, "code", None) is not None:
